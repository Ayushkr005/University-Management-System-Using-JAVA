...
public class StudentFeeForm extends JFrame implements ActionListener {
    ...
    StudentFeeForm() {
        ...
        // Populate roll numbers from database
        try {
            Conn c = new Conn();
        ...
        // Populate Name and Father's Name based on selected roll number
        ...
        String[] semester = {"1st_Year", "2nd_Year", "3rd_Year", "4th_Year"};
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == update) {
            ...
            try {
                Conn c = new Conn();
                ResultSet rs = c.s.executeQuery("select * from amount where course = '" + course + "'");
                if (rs.next()) { // Check if a row is returned
                    labeltotal.setText(rs.getString(semester)); // Assuming `semester` is a valid column name
                } else {
                    JOptionPane.showMessageDialog(null, "No fee data found for selected course and semester");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
